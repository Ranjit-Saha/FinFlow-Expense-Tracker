...
def get_expenses(expense_date: date):
    expenses = db_helper.fetch_expenses_for_a_given_date(expense_date)
    if expenses is None:
...
def add_or_update_expense(expense_date: date, expenses: List[Expense]):
    db_helper.delete_expenses_for_a_given_date(expense_date)
...
def get_analytics():
    monthly_summary = db_helper.get_expenses_by_month()
